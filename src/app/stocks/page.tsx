  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        ...
        {['Technology', 'Healthcare', 'Financial Services', 'Consumer Goods', 'Automotive', 'E-commerce'].map((sectorName) => (
          <button
            key={sectorName}
            onClick={() => window.location.href = `/stocks?sector=${sectorName.toLowerCase()}`}
            className={`px-3 py-2 rounded-full text-sm font-medium transition-colors ${
      ...
      {filteredStocks.length > 0 ? (
        ...
        <Card>
          <div className="text-center py-8">
            <p className="text-gray-600">No stocks found for the selected criteria.</p>
          </div>
        </Card>
      )}
    </div>
